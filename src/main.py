...
class PredictMood():
    ...
    def rf_data_categorization_preparation(self, impute_option=ML_IMPUTE, impute_strategy=RF_REGRESSOR):
        self.rf_input_df = self.feature_maker.build_non_temporal_dataset_from_cleaned(self.clean_df, impute_option, impute_strategy)
        print("Generating RF data file imputed with impute option: ",impute_option, "and impute strategy: ",impute_strategy)
    ...
    def rnn_classifier_run(self, impute_option=MEDIAN_IMPUTE, impute_strategy=RBF_BAYESIAN_RIDGE, production_run=True):
        for seqs, labels in self.feature_maker.build_rnn_temporal_dataset(self.clean_df, impute_option, impute_strategy):
